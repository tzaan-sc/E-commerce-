...
const Header = () => {
  ...
  // 3. RENDER DROPDOWN (ĐÃ SỬA LOGIC LINK)
  const renderDropdown = (menu) => {
    if (!menu.child || menu.child.length === 0) return null;
    const hasColumns = menu.child.length > 0 && menu.child[0].subchild;

    if (hasColumns) {
      // 👇 QUAN TRỌNG: Xác định trang Laptop cơ sở (User hay Customer)
      // Nếu là Customer -> /customer/home/laptop
      // Nếu là Khách -> /laptop
            ...
            <li key={colKey} className="dropdown-column">
              ...
              <ul className="sub-dropdown">
                {column.subchild.map((item, itemKey) => {
                    // 👇 TẠO LINK QUERY PARAM
                    let linkTo = item.path; // Mặc định lấy từ data

                    // Nếu là menu THƯƠNG HIỆU -> /laptop?brand=1
                    if (column.name.toUpperCase().includes("THƯƠNG HIỆU") && item.id) {
                        linkTo = `${laptopBasePath}?brand=${item.id}`;
                    }
                    // Nếu là menu NHU CẦU -> /laptop?purpose=2
    ...
    // Dropdown Tài khoản (Giữ nguyên logic lọc)
    return (
